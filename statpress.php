...
if (is_admin())
  ...
  // a custom function for loading localization
	...
	{   //check whether necessary core function exists
		...
		load_plugin_textdomain('statpressV', 'wp-content/plugins/' . dirname(plugin_basename(__FILE__)) . '/locale');
		}
	}
    ...
    function luc_StatPress_TopPosts($limit = 5, $showcounts = 'checked')
      { global $wpdb;
        $res = "\n<ul>\n";
        $table_name = $wpdb->prefix . "statpress";
      ...
      { if (!function_exists('register_sidebar_widget') || !function_exists('register_widget_control'))
        ...
        function widget_statpressV_control()
          {   $options = get_option('widget_statpressV');
              if (!is_array($options))
                  $options = array('title' => 'statpress V', 'body' => 'Visitors today: %todayvisitors%');
              ...
              echo '<input type="hidden" id="statpress-submit" name="statpress-submit" value="1" />
        ...
        function widget_statpressV($args)
          ...
          register_sidebar_widget('statpress V', 'widget_statpressV');
          register_widget_control(array('statpress V', 'widgets'), 'widget_statpressV_control', 300, 210);
          
          // Top posts
        function widget_statpressVtopposts_control()
          {   $options = get_option('widget_statpressVtopposts');
              if (!is_array($options))
                 { $options = array('title' => 'StatPress V TopPosts', 'howmany' => '5', 'showcounts' => 'checked');
                 }
              ...
              // the form
              echo '<p style="text-align:right;"><label for="statpresstopposts-title">' . __('Title', 'statpressV') . ' <input style="width: 250px;" id="statpress-title" name="statpresstopposts-title" type="text" value="' . $title . '" /></label></p>';
              echo '<p style="text-align:right;"><label for="statpresstopposts-howmany">' . __('Limit results to', 'statpressV') . ' <input style="width: 100px;" id="statpresstopposts-howmany" name="statpresstopposts-howmany" type="text" value="' . $howmany . '" /></label></p>';
              echo '<p style="text-align:right;"><label for="statpresstopposts-showcounts">' . __('Visits', 'statpressV') . ' <input id="statpresstopposts-showcounts" name="statpresstopposts-showcounts" type=checkbox value="checked" ' . $showcounts . ' /></label></p>';
        ...
        function widget_statpressVtopposts($args)
          {
              ...
              echo $before_widget;
              echo($before_title . $title . $after_title);
              echo luc_StatPress_TopPosts($howmany, $showcounts);
              echo $after_widget;
          }
       register_sidebar_widget('StatPress V TopPosts', 'widget_statpressVtopposts');
       register_widget_control(array('StatPress V TopPosts', 'widgets'), 'widget_statpressVtopposts_control', 300, 110);
       }
