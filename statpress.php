...
<?php
	...
	<tbody id='the-list'>";
          ...
          //###############################################################################################
          // VISITORS FEEDS ROW
         print "<tr><td><div style='background:$rss_visitors_color;width:10px;height:10px;float:left;margin-top:4px;margin-right:5px;'></div>" . __('Visitors Feeds', 'dailystat') . "</td>";
          ...
          print "</tr>";
		   //###############################################################################################
          // PAGEVIEWS FEEDS ROW
         print "<tr><td><div style='background:$rss_posts_color;width:10px;height:10px;float:left;margin-top:4px;margin-right:5px;'></div>" . __('Pageviews Feeds', 'dailystat') . "</td>";
...
echo 'le script s\'execute en : '.round($temps_fin - $temps_deb .'micro secondes', 4);    
	  ...
	  {  global $wpdb;
         ...
         $spider_color = "#83b4d8";
		 $yesterday = gmdate('Ymd', current_time('timestamp') - 86400);
			$pageArticles = luc_page_articles();
			...
			if ($action == "yesterday") 
		      {
				print "<div class='wrap'><h2>" . __('Yesterday ', 'dailystat').gmdate('d M, Y', current_time('timestamp')-86400) ."</div>";
			   luc_printpagelink ($action,$NumberOfPagesArticles,$pageArticles);
			   
				print "<table class='widefat'><thead><tr>
	
	<th scope='col'>". __('URL','dailystat'). "</th>
	<th scope='col'><div style='background:$unique_color;width:10px;height:10px;float:left;margin-top:4px;margin-right:5px;'></div>". __('Visitors','dailystat'). "<br /><font size=1></font></th>
	...
	<th scope='col'><div style='background:$web_color;width:10px;height:10px;float:left;margin-top:4px;margin-right:5px;'></div>". __('Views','dailystat'). "<br /><font size=1></font></th>
	<th scope='col'><div style='background:$rss_posts_color;width:10px;height:10px;float:left;margin-top:4px;margin-right:5px;'></div>". __('Views Feeds','dailystat'). "<br /><font size=1></font></th>
	...
	<th scope='col'>". __('Total','dailystat'). "<br /><font size=1></font></th>
			  ...
			  if ($action == 'yesterday') // sort post or page by most views
				    {   $qry_visitors =$wpdb->get_results("SELECT  post_name, total
				        ...
				        $qry_visitors_feeds=$wpdb->get_results("SELECT post_name, total
					     FROM
						   ((SELECT 'page_accueil' as post_name, count(distinct ip) AS total
	                         FROM $table_name 
						...
						$qry_pageviews=$wpdb->get_results("SELECT post_name, total, urlrequested
					       FROM
						...
						UNION 
						    (SELECT post_name, NULL as total, urlrequested
                               FROM	$wpdb->posts as p 
							   JOIN $table_name as t ON t.urlrequested LIKE CONCAT('%', p.post_name, '%' )
						...
						$qry_pageviews_feeds=$wpdb->get_results("SELECT post_name, total
					     FROM
			...
			if ($action == "yesterday") 	   
	           {    foreach ($qry_visitors as $url)
				   ...
				   {$spiders[$url->post_name]=$url->total;
				    $totalspiders += $url->total;
			   }
			  print "<th scope='col'>All URL</th>"; 
			   print "<th scope='col'>". __($totalvisitors,'dailystat'). "</th>";
			   print "<th scope='col'>". __($total_visitors_feeds,'dailystat'). "</th>";
			   print "<th scope='col'>". __($total_pageviews,'dailystat'). "</th>";
			   print "<th scope='col'>". __($total_pageviews_feeds,'dailystat'). "</th>";
			   print "<th scope='col'>". __($totalspiders,'dailystat'). "</th>";
			   print "</tr></div>";
			  foreach ($qry_pageviews as $url)
			   {
               print "<tr><td>". get_bloginfo('url'). $url->urlrequested ."</td>"; 
			   print "<td>" . $visitors[$url->post_name] . "</td>\n";
			   print "<td>" . $visitors_feeds[$url->post_name] . "</td>\n";
			   print "<td>" . $pageviews[$url->post_name] . "</td>\n";
			   print "<td>" . $pageviews_feeds[$url->post_name] . "</td>\n";
		     ...
		     // $action == referrer
			  else {$query = $wpdb->get_results($strqry);
			        foreach ($query as $url)
			       {print "<tr><td><h4><a href='" . $url->referrer. "' target='_blank'>$url->referrer</a></h4></td>";
...
%topbrowser% %topos% %pagestoday% %thistotalpages% %latesthits%</div>';
    ...
    function dailystat_load_textdomain() 
	{
      ...
      //add_action('wp_head', 'luc_StatAppend');
