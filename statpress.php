	 ...
	 function luc_yesterday()
	  { global $wpdb;	 
		...
		$qry_total_visitors = $wpdb->get_row("SELECT count(distinct ip) AS total
                                              FROM $table_name
                                              WHERE feed='' AND spider='' AND date = $yesterday");
		$qry_visitors = requete_yesterday ("count(distinct ip) as total","urlrequested ='' AND spider='' AND feed=''  AND date = $yesterday",
		                                   "post_status = 'publish' AND (post_type = 'page' OR post_type = 'post') AND spider='' AND feed='' AND date = $yesterday  ");	
		
		$qry_total_visitors_feeds = $wpdb->get_row("SELECT count(distinct ip) as total
                                                    FROM $table_name
                                                    WHERE feed<>'' AND spider=''   AND date = $yesterday");
		
		$qry_visitors_feeds = requete_yesterday ("count(distinct ip) as total","(urlrequested LIKE '".$permalink[0]."feed%' OR urlrequested LIKE '".$permalink[0]."comments/feed%' AND spider='' AND feed<>'' AND date = $yesterday",
		                                         "p.post_status = 'publish'  AND (post_type = 'page' OR post_type = 'post') AND spider='' AND feed<>'' AND date = $yesterday");					
		...
		$qry_pageviews=$wpdb->get_results("SELECT post_name, total, urlrequested
					                       FROM
						                    ((SELECT 'page_accueil' as post_name, count(ip) as total, urlrequested
							                  FROM $table_name 
									          WHERE urlrequested ='' AND spider='' AND feed='' AND date = $yesterday
									          GROUP BY post_name)
                                           UNION ALL
						                     (SELECT post_name, count(ip) as total, urlrequested
		                                      FROM $wpdb->posts as p
		                                      JOIN $table_name as t
                                              ON urlrequested LIKE CONCAT('".$permalink[0]."', p.post_name, '_' )
		                                      WHERE post_status = 'publish' AND (post_type = 'page' OR post_type = 'post') AND spider='' AND feed='' AND date = $yesterday
							                  GROUP BY post_name) 
						                   UNION 
						                     (SELECT post_name, NULL as total, urlrequested
                                              FROM	$wpdb->posts as p 
							                  JOIN $table_name as t ON t.urlrequested LIKE CONCAT('".$permalink[0]."', p.post_name, '_' ) 
							                  WHERE  p.post_status = 'publish' AND (post_type = 'page' OR post_type = 'post') 
							                  GROUP BY post_name)
							                ) views		 
						                   GROUP BY post_name 
		...
		$qry_pageviews_feeds = requete_yesterday ("count(ip) as total","(urlrequested LIKE '".$permalink[0]."feed%' OR urlrequested LIKE '".$permalink[0]."comments/feed%')
						                            AND spider='' AND feed<>'' AND date = $yesterday","p.post_status = 'publish' AND (post_type = 'page' OR post_type = 'post') AND spider='' AND feed<>'' AND date = $yesterday");					
												 
		$qry_spiders = requete_yesterday ("count(ip) as total","urlrequested ='' AND spider<>'' AND feed=''  AND date = $yesterday",
		                                  "post_status = 'publish' AND (post_type = 'page' OR post_type = 'post') AND date = $yesterday AND spider<>'' AND feed=''");	
