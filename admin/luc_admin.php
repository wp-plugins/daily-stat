<?php
  ...
  function luc_add_pages()
  {
      ...
      $table_name = $wpdb->prefix. 'dailystat';
       ...
       if (($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) or (get_option('dailystat_dbversion') <> '1.4'))
		   ...
		   echo "</table>";
            ...
            $querylimit = "LIMIT 50";
             ...
             {echo "<tr>
			  ...
			  echo "<td>" . luc_dailystat_Abbrevia(luc_dailystat_Decode($rk->urlrequested), 50) . "</td>";
			  luc_image_OBS($rk,'os','os.dat','os');
              echo "<td>" . $rk->os . "</td>";
			  luc_image_OBS($rk,'browser','browser.dat','browsers');
              echo "<td>" . $rk->browser . "</td>";
            ...
            foreach ($qry as $rk)
             {
			  ...
			  luc_language($rk,'language','languages.dat');
			  luc_image_NC($rk,'country','domain.dat','domain',strtolower($rk->country)<>$rk->nation);
			  echo "<td><a href='" . $rk->referrer . "'>" . luc_dailystat_Abbrevia(luc_dailystat_Decode($rk->referrer), 100) . "</a></td>";
			  echo "<td>" . luc_dailystat_Abbrevia(luc_dailystat_Decode($rk->urlrequested), 50) . "</td>
			  </tr>\n";
             }
            echo "</tbody></table>";
            ...
            foreach ($qry as $rk)
             {
              ...
              echo "<td>" . luc_dailystat_Abbrevia(luc_dailystat_Decode($rk->urlrequested), 80) . "</td>
              <td> " . $rk->agent . "</td></tr>\n";
             }
